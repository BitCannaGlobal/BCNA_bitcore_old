    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat BitCanna and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002BitCanna per transaction on average.
            */
            if(r <= 20)
            {
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeBitCannaAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fBitCannaSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f BitCanna CENT - sessionTotalValue %d\n", fBitCannaSubmitted, sessionTotalValue);
...
void CDarkSendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100BitCanna+1 ( bit on if present )
    // bit 1 - 10BitCanna+1
    // bit 2 - 1BitCanna+1
    // bit 3 - .1BitCanna+1
    // bit 3 - non-denom
...
void ThreadCheckDarkSendPool()
{
    ...
    RenameThread("BitCanna-darksend");
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeBitCannaAmount = (rand() % nLeftToAnon)+3;
                } else {
