...
QString TransactionDesc::toHTML(CWallet* wallet, CWalletTx& wtx, TransactionRecord* rec, int unit)
{
        ...
        if (nNet > 0) {
            ...
            if (CBitcoinAddress(rec->address).IsValid()) {
                ...
                if (wallet->mapAddressBook.count(address)) {
                    CTxOut PrevOut = getPrevOut(wtx.vin[0].prevout);
                    std::string addressFrom = ScriptToString(PrevOut.scriptPubKey, true);
                    strHTML += "<b>" + tr("From") + ":</b> " + GUIUtil::HtmlEscape(addressFrom) + "<br>";
                    strHTML += "<b>" + tr("To") + ":</b> ";
    ...
    // Message from normal bitcanna:URI (bitcanna:XyZ...?message=example)
