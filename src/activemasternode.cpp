...
//
// Bootup the masternode, look for a 500 BitCanna input and register on the network
//
...
vector<COutput> CActiveMasternode::SelectCoinsMasternode() {
    ...
    BOOST_FOREACH(const COutput& out, vCoins) {
        if(out.tx->vout[out.i].nValue == GetMNCollateral(chainActive.Tip()->nHeight)*COIN) {  //exactly 16120 BCNA
...
//    scriptPubKey.SetDestination(address.Get());
    ...
    // Filter
    BOOST_FOREACH(const COutput& out, vCoins) {
        if(out.tx->vout[out.i].scriptPubKey == scriptPubKey && out.tx->vout[out.i].nValue == 16120*COIN) { //exactly 161.200 BCNA
        	filteredCoins.push_back(out);
        }
    }
    return filteredCoins;
