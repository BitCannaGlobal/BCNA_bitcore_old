...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BCNA address (e.g. %1)").arg("LhfHsuK9Ekjj2dYoEy4wvEiQHvRtzZQnYa"));
#endif
...
{
    // return if URI is not valid or is no BCNA: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::BCNA, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert bitcanna:// to bitcanna:
    //
    //    Cannot handle this later, because bitcanna:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BCNA, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open bitcanna.conf with the associated application */
                                                                                                                                          ...
                                                                                                                                          allColumnsMinimumWidth(allColsMinimumWidth)
{
    ...
    if(secondToLastColumnIndex == 0) { // if only 2 columns
        setViewHeaderResizeMode(secondToLastColumnIndex, QHeaderView::Fixed);
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BCNA.lnk";
}
...
{
    return GetAutostartDir() / "bitcanna.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a bitcanna.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=BCNA\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the bitcanna app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add bitcanna app to startup item list
