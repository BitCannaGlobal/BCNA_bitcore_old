...
// Implemented BCNA parallel miner | Auto deploy
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BCNAMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("BCNAMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("BCNAMiner started (%s)\n", (fProofOfStake ? "PoS" : "PoW"));
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcanna-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running BCNAMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
