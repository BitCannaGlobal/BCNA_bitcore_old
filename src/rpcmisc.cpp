...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total bitcanna balance of the wallet\n"
            "  \"darksend_balance\": xxxxxx, (numeric) the anonymized bitcanna balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in bitcanna/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in bitcanna/kb\n"
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"bitcannaaddress\"\n"
            "\nReturn information about the given bitcanna address.\n"
            "\nArguments:\n"
            "1. \"bitcannaaddress\"     (string, required) The bitcanna address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"bitcannaaddress\", (string) The bitcanna address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: BCNA address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are bitcanna addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) bitcanna address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"bitcannaaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"bitcannaaddress\"  (string, required) The bitcanna address to use for the signature.\n"
