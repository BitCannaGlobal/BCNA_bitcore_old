...
void CWallet::AvailableCoinsMN(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, AvailableCoinsType coin_type, bool useIX) const
{
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            if (nDepth <= 0) // BitCannaNOTE: coincontrol fix / ignore 0 confirm
                continue;
...
bool CWallet::SelectCoins(const std::string &account, const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 BCNA
    if (coin_type == ONLY_DENOMINATED) {
        ...
        BOOST_FOREACH (int64_t v, darkSendDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                ...
                if (txout.nValue == v //make sure it's the denom we're looking for
                    && nValueRet + txout.nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 BCNA over
                    ) {
...
bool CWallet::SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, int64_t& nValueRet, int nDarksendRoundsMin, int nDarksendRoundsMax)
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            //
            // bit 0 - 10000 BCNA+1 ( bit on if present )
            // bit 1 - 1000 BCNA+1
            // bit 2 - 100 BCNA+1
            // bit 3 - 10 BCNA+1
            // bit 4 - 1 BCNA+1
            ...
            while (true) {
                ...
                if (!SelectCoins(wtxNew.strFromAccount, nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 BCNA.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOTMN) {
                        strFailReason = _("Unable to locate enough DarkSend non-denominated funds for this transaction that are not equal 10000 BCNA.");
                    } else {
...
#               endif
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-bitcanna-address
                    CScript scriptChange;
...
bool CWallet::AddBitCannaNodeConfig(CBitCannaNodeConfig nodeConfig)
{
    bool rv = CWalletDB(strWalletFile).WriteBitCannaNodeConfig(nodeConfig.sIndex, nodeConfig);
    if(rv)
        uiInterface.NotifyBitCannaNodeChanged(nodeConfig);
