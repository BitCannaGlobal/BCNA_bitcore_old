...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered bitcanna address / amount:
...

    {
        ...
        if (recipients[0].useInstanTX && total > GetSporkValue(SPORK_2_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("InstanTX doesn't support sending values that high yet. Transactions are currently limited to %1 BCNA.").arg(GetSporkValue(SPORK_2_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal bitcanna:URI (bitcanna:XyZ...?message=example)
            {
        ...
        if (!pwalletMain->GetKeyFromPool(newKey))
            return ErrorGettingWalletAddress;
        CKeyID keyID = newKey.GetID();

        pwalletMain->SetAddressBook(keyID, strAccount, "receive");

        newTx->mapValue["from"] = CBitcoinAddress(keyID).ToString();
