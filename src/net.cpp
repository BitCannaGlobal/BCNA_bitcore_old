...
bool CNode::IsBanned(CNetAddr ip)
{
    bool fResult = false;
    {
        ...
        if (i != setBanned.end()) {
            int64_t t = (*i).second;
            if (GetTime() < t)
                LogPrintf("this addr has ban %s \n ",ip.ToString());
                fResult = true;
        }
    }
    if(!fResult) LogPrintf("this addr not ban %s \n",ip.ToString());
    return fResult;
}
...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (BCNA users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "BCNA " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Bitcanna is probably already running."), addrBind.ToString());
        else
