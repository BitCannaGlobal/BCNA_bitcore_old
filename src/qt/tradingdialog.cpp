        ...
        model(0)
{
    ...
    ui->BCNAAvailableLabel->setTextFormat(Qt::RichText);
    ...
    ui->CryptopiaBCNALabel->setTextFormat(Qt::RichText);
...
//    connect(ui->TradingTabWidget, SIGNAL(tabBarClicked(int)), this, SLOT(on_TradingTabWidget_tabBarClicked(int)));
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "SUM(BTC)" << "TOTAL(BTC)" << "BCNA(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)" << "BCNA(SIZE)" << "TOTAL(BTC)" << "SUM(BTC)");
    ...
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(BCNA)"<<"TOTAL COST(BTC");
...
void tradingDialog::UpdaterFunction(){
    //BCNAst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
    ...
    QString Response = sendRequest("https://www.cryptopia.co.nz/api/GetMarket/BCNA_BTC");
    return Response;
}

QString tradingDialog::GetOrderBook(){

    QString  Response = sendRequest("https://www.cryptopia.co.nz/api/GetMarketOrders/BCNA_BTC");
    return Response;
}
...
void tradingDialog::GetMarketHistory(){
    ...
    sendRequest1(QString("https://www.cryptopia.co.nz/api/GetMarketHistory/BCNA_BTC"), f);
}
...
QString tradingDialog::BuyBCNA(QString OrderType, double Quantity, double Rate){

    QString str = "";
    QString URL = "https://www.cryptopia.co.nz/api/SubmitTrade";

    QJsonObject stats_obj;
    stats_obj["Market"] = "BCNA/BTC";
    stats_obj["Type"] = "Buy";
...
QString tradingDialog::SellBCNA(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString Response = sendRequest(URL, "POST", QString("{\"Currency\":\"BCNA\"}"));
    return Response;
}
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>BCNA Volume:</b> <span style='font-weight:bold; font-size:12px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> BCNA");
...
void tradingDialog::AskInfoInsertSlot(int row, int col){
    ...
    QString UnitsBCNAString = ui->AsksTable->model()->data(ui->AsksTable->model()->index(row,1)).toString();
    ui->SellBidPriceEdit->setText(SellBidPriceString);
    ui->UnitsInputBCNA->setText(UnitsBCNAString);
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    ui->BCNASupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(BCNASupply,'i',8) + "</span><b> BCNA</b>");
    ...
    ui->BCNADemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(BCNADemand,'i',8) + "</span><b> BCNA</b>");
...
void tradingDialog::ParseAndPopulatePriceChart(QString Response){
    ...
    derPlot->yAxis->setLabel("BCNA Price in BTC");
...
void tradingDialog::ActionsOnSwitch(int index = -1){
    ...
    switch (index){
        case 0:    //buy tab is active
            ...
            f = std::bind(&tradingDialog::showBalanceOfBCNAOnTradingTab, this);
            sendRequest1(QString("https://www.cryptopia.co.nz/api/GetBalance"), f, "POST", QString("{\"Currency\":\"") + "BCNA" + QString("\"}"));
            ...
            sendRequest1("https://www.cryptopia.co.nz/api/GetMarketOrders/BCNA_BTC", f);
        ...
        case 1: //Cross send tab active
            ...
            f = std::bind(&tradingDialog::showBalanceOfBCNAOnSendTab, this);
            sendRequest1(QString("https://www.cryptopia.co.nz/api/GetBalance"), f, "POST", QString("{\"Currency\":\"") + "BCNA" + QString("\"}"));
        ...
        case 5://show balance tab
            ...
            f = std::bind(&tradingDialog::showBalanceOfBCNAOnBalanceTab, this);
            sendRequest1(QString("https://www.cryptopia.co.nz/api/GetBalance"), f, "POST", QString("{\"Currency\":\"") + "BCNA" + QString("\"}"));
...
void tradingDialog::showBalanceOfBCNAOnTradingTab() {
    QNetworkReply *reply = qobject_cast<QNetworkReply *>(sender());
    QString Response = "";
    if (reply->error() == QNetworkReply::NoError) {
        //success
    ...
    reply->deleteLater();
    if(Response.size() > 0 && Response != "Error"){
        DisplayBalance(*ui->BCNAAvailableLabel, Response);
    }
}
...
void tradingDialog::showBalanceOfBCNAOnSendTab() {
    QNetworkReply *reply = qobject_cast<QNetworkReply *>(sender());
    QString Response = "";
    if (reply->error() == QNetworkReply::NoError) {
        //success
    ...
    reply->deleteLater();
    if(Response.size() > 0 && Response != "Error"){
        DisplayBalance(*ui->CryptopiaBCNALabel, Response);
    }
}
...
void tradingDialog::showBalanceOfBCNAOnBalanceTab() {
    QNetworkReply *reply = qobject_cast<QNetworkReply *>(sender());
    QString Response = "";
    if (reply->error() == QNetworkReply::NoError) {
        //success
    ...
    reply->deleteLater();
    if(Response.size() > 0 && Response != "Error"){
        DisplayBalance(*ui->BCNABalanceLabel,*ui->BCNAAvailableLabel_2,*ui->BCNAPendingLabel, QString::fromUtf8("BCNA"),Response);
    }
}
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputBCNA->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
void tradingDialog::CalculateCSReceiveLabel(){
    ...
    QString balance = GetBalance("BCNA");
    ...
    double AvailableBCNA = BalanceObject["Available"].toDouble();
...
{
    //calculate amount of BTC that can be gained from selling BCNA available balance
    QString responseA = GetBalance("BCNA");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONArray(responseA);

    double AvailableBCNA = ResultObject["Available"].toDouble();

    ui->UnitsInputBCNA->setText(str.number(AvailableBCNA,'i',8));
}
...
{
    double Quantity = ui->CryptopiaBCNALabel->text().toDouble();
    double Received = 0;
    double Qty = 0;
    double Price = 0;
    QString buyorders = GetOrderBook();
...
void tradingDialog::on_Withdraw_Max_Amount_clicked()
{
    ...
    ui->WithdrawUnitsInput->setText(str.number(AvailableBCNA,'i',8));
}
...
void tradingDialog::on_BuyBCNA_clicked()
{
    double Rate;
    double Quantity;
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  BuyBCNA(Order,Quantity,Rate);
...
void tradingDialog::on_SellBCNABTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputBCNA->text().toDouble();
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  SellBCNA(Order,Quantity,Rate);
        QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
        QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
...
void tradingDialog::on_CSUnitsBtn_clicked()
{
    ...
    foreach (const QJsonValue & value, BuyArray)
    {
        ...
        if ( ((Quantity / x) - y) > 0 )
        {
            ...
            QString SellResponse = SellBCNA(Order,y,x);
            ...
            QString SellResponse = SellBCNA(Order,(Quantity / x),x);
            QJsonDocument SelljsonResponse = QJsonDocument::fromJson(SellResponse.toUtf8());          //get json from str.
            QJsonObject SellResponseObject = SelljsonResponse.object();                              //get json obj
            ...
            if (SellResponseObject["Success"].toBool() == false){
                ...
                if (ResponseObject["Success"].toBool() == false){
                    ...
                    if (ResponseObject["Success"].toBool() == false){
                        ...
                        QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" BCNA for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                    }
                } else if (ResponseObject["Success"].toBool() == true){
                    QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" BCNA for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                }
...
void tradingDialog::on_UnitsInputBCNA_textChanged(const QString &arg1)
{
    CalculateSellCostLabel(); //update cost
}
